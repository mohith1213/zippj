...
export default function CheckerApplicationReview() {
  ...
  useEffect(() => {
    const mapDtoToReview = (dto) => {
      const fd = dto.formData || {};
      const nameParts = String(fd.fullName || '').split(' ');
      const firstName = nameParts[0] || '';
      const middleName = nameParts.length > 2 ? nameParts.slice(1, -1).join(' ') : '';
      const lastName = nameParts.length > 1 ? nameParts[nameParts.length - 1] : '';
      return {
        id: dto.id,
        status: (dto.status || '').toLowerCase() === 'with checker' ? 'pending' : (dto.status || '').toLowerCase(),
        ...
        personalDetails: {
          ...
          highestQualification: fd.highestQualification || '',
        ...
        employmentDetails: {
          ...
          totalWorkExperience: fd.totalExperience || '',
        ...
        loanDetails: {
          ...
          cibilScore: fd.cibilScore ? Number(fd.cibilScore) : 700,
        },
        existingLoanDetails: {
          loanType: fd.existingLoanType || '',
          lender: fd.existingLender || '',
          outstandingAmount: Number(fd.outstandingAmount || 0),
          emi: Number(fd.existingEmi || 0),
          tenureRemaining: fd.tenureRemaining || '',
        },
        documents: {
          photograph: { name: 'Photograph', uploaded: !!fd.photograph, type: 'image', url: '#' },
          idProof: { name: 'ID Proof', uploaded: !!fd.idProof, type: 'pdf', url: '#' },
          addressProof: { name: 'Address Proof', uploaded: !!fd.addressProof, type: 'pdf', url: '#' },
          cibilReport: { name: 'CIBIL Report', uploaded: !!fd.cibilReport, type: 'pdf', url: '#' },
          paySlip: { name: 'Payslip', uploaded: !!fd.salariedPayslip, type: 'pdf', url: '#' },
          employmentProof: { name: 'Employment Proof', uploaded: !!fd.salariedEmploymentProof, type: 'pdf', url: '#' },
          itr: { name: 'ITR', uploaded: !!fd.salariedItr || !!fd.selfItr, type: 'pdf', url: '#' },
          gst: { name: 'GST Registration', uploaded: !!fd.selfGst, type: 'pdf', url: '#' },
          bankStatements: { name: 'Bank Statements', uploaded: !!fd.selfBankStatements, type: 'pdf', url: '#' },
          homeEc: { name: 'Encumbrance Certificate', uploaded: !!fd.homeEc, type: 'pdf', url: '#' },
          vehicleInvoice: { name: 'Vehicle Invoice', uploaded: !!fd.vehicleInvoice, type: 'pdf', url: '#' },
        },
        makerComments: [],
      };
    };
    ...
    const load = async () => {
      try {
        const dto = await getApplication(id);
        setApplicationData(mapDtoToReview(dto));
      } catch {
  ...
  const confirmDecision = async () => {
    ...
    try {
      if (confirmationType === "approve") {
        ...
        await rejectApplication(id, rejectionComment);
      }
      const title = confirmationType === "approve" ? "Approved" : "Rejected";
